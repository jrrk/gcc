...
record_address_taken_vars (void)
{
  ...
  FOR_EACH_BB (bb)
    {
      for (csi = cil_iter_start_bb (bb); !cil_iter_end_p (csi); cil_iter_next (&csi))
	{
	  stmt = cil_iter_stmt (csi);
		       ...
		       bool address_taken)
{
  ...
  for (cil_iter_next (&csi); !cil_iter_end_p (csi); cil_iter_next (&csi))
    {
      stmt = cil_iter_stmt (csi);
		  ...
		  if (INTEGRAL_TYPE_P (TREE_TYPE (var)))
		    {
			 ...
			 allows for the variable to be removed.  */
		      ...
		      if (conv)
			{
			  cil_iter_insert_before (&csi, conv, CIL_ITER_SAME_STMT);
			  cil_stack_after_stmt (stack, conv);
			}
		    }

		  dup = cil_build_stmt (CIL_DUP);
		  cil_iter_insert_before (&csi, dup, CIL_ITER_SAME_STMT);
...
propagate_copy (cil_stmt store, tree dst, tree src, bool arg)
{
  ...
  csi = cil_iter_for_stmt (store);
  cil_iter_next (&csi);

  while (!cil_iter_end_p (csi))
    {
      stmt = cil_iter_stmt (csi);
      opcode = cil_opcode (stmt);
      ...
      while (!cil_iter_end_p (csi))
	{
	  ...
	  if ((cil_opcode (stmt) == CIL_STLOC)
	      ...
	      if (!cil_iter_first_p (csi))
		{
		  ...
		  else if (cil_conversion_p (stmt) && !cil_iter_first_p (csi))
		    {
		      cil_iter_prev (&csi);
		      stmt = cil_iter_stmt (csi);
		  ...
		  cil_iter_next (&csi);
		  cil_iter_replace (&csi, cil_build_stmt (CIL_POP));
		  cil_iter_next (&csi);
		}
	      else
		{
