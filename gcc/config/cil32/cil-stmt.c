...
static void cil_iter_insert_seq_nodes_before (cil_stmt_iterator *, cil_seq_node,
					 cil_seq_node,
					 enum cil_iter_iterator_update);
static void cil_iter_insert_seq_nodes_after (cil_stmt_iterator *, cil_seq_node,
					cil_seq_node, enum cil_iter_iterator_update);
...
{
  cil_stmt stmt = XCNEW (struct cil_stmt_d);
  cil_switch_arg arg = XCNEW (struct cil_switch_arg_d);
  unsigned int i, length = TREE_VEC_LENGTH (labels);

  gcc_assert (length != 0);
  arg->ncases = length;
  arg->cases = XCNEWVEC (tree, length);
...
{
  cil_stmt stmt = XCNEW (struct cil_stmt_d);
  cil_call_arg arg = XCNEW (struct cil_call_arg_d);
  tree arg_types;
  size_t i;
  ...
  if (arg_types == NULL)
  ...
  else
    {
      ...
      arg->arguments = XCNEWVEC (tree, arg->nargs);
...
cil_copy_stmt (const_cil_stmt stmt)
{
  ...
  new_stmt = XCNEW (struct cil_stmt_d);
  ...
  if (opcode_arg_type (stmt->opcode) == CIL_LABELS)
    {
      ...
      arg->ncases = ncases;
      arg->cases = XCNEWVEC (tree, ncases);
  ...
  else if (opcode_arg_type (stmt->opcode) == CIL_FCALL)
    {
      ...
      if (nargs != 0)
	{
	  arg->arguments = XCNEWVEC (tree, nargs);
...
cil_seq_add_stmt (cil_seq *seq_p, cil_stmt cs)
{
  ...
  si = cil_iter_last (*seq_p);
  cil_iter_insert_after (&si, cs, CIL_ITER_NEW_STMT);
}
...
cil_seq_add_seq (cil_seq *dst_p, cil_seq src)
{
  ...
  si = cil_iter_last (*dst_p);
  cil_iter_insert_seq_after (&si, src, CIL_ITER_NEW_STMT);
}
...
cil_seq_deep_copy (cil_seq src)
{
  ...
  for (csi = cil_iter_start (src); !cil_iter_end_p (csi); cil_iter_next (&csi))
    {
      stmt = cil_copy_stmt (cil_iter_stmt (csi));
...
cil_set_bb_seq (basic_block bb, cil_seq seq)
{
  ...
  if (*slot == NULL)
    {
      cil_basic_block cbb = XCNEW (struct cil_basic_block_d);
...
cil_seq_stack_depth (cil_seq seq, unsigned int init, bool max)
{
  ...
  for (i = cil_iter_start (seq); !cil_iter_end_p (i); cil_iter_next (&i))
    {
      cil_stmt cs = cil_iter_stmt (i);
...
cil_seq
cil_iter_split_seq_after (cil_stmt_iterator i)
{
...
cil_seq
cil_iter_split_seq_before (cil_stmt_iterator *i)
{
...
void
cil_iter_replace (cil_stmt_iterator *csi, cil_stmt stmt)
{
  cil_stmt orig_stmt = cil_iter_stmt (*csi);

  if (stmt == orig_stmt)
    return;

  cil_set_locus (stmt, cil_locus (orig_stmt));
  cil_set_bb (stmt, cil_iter_bb (*csi));

  *cil_iter_stmt_ptr (csi) = stmt;
}
...
/* Insert statement STMT before the statement pointed-to by iterator
   ...
   I after insertion (see enum cil_iter_iterator_update).  */

void
cil_iter_insert_before (cil_stmt_iterator *i, cil_stmt stmt,
		   enum cil_iter_iterator_update m)
{
  cil_seq_node n;

  n = XCNEW (struct cil_seq_node_d);
  n->prev = n->next = NULL;
  n->stmt = stmt;
  cil_iter_insert_seq_nodes_before (i, n, n, m);
}
...
/* Inserts the sequence of statements SEQ before the statement pointed
   ...
   insertion (see enum cil_iter_iterator_update).  */

void
cil_iter_insert_seq_before (cil_stmt_iterator *i, cil_seq seq,
		       enum cil_iter_iterator_update m)
{
  ...
  cil_iter_insert_seq_nodes_before (i, first, last, m);
}
...
/* Insert statement STMT after the statement pointed-to by iterator I,
   ...
   after insertion (see enum cil_iter_iterator_update).  */

void
cil_iter_insert_after (cil_stmt_iterator *i, cil_stmt stmt,
		  enum cil_iter_iterator_update m)
{
  cil_seq_node n;
  ...
  n->prev = n->next = NULL;
  n->stmt = stmt;
  cil_iter_insert_seq_nodes_after (i, n, n, m);
}

/* Links sequence SEQ after the statement pointed-to by iterator I.
   M is as in cil_iter_insert_after.  */

void
cil_iter_insert_seq_after (cil_stmt_iterator *i, cil_seq seq,
		      enum cil_iter_iterator_update m)
{
  cil_seq_node first, last;
  ...
  cil_iter_insert_seq_nodes_after (i, first, last, m);
}
...
void
cil_iter_remove (cil_stmt_iterator *i)
{
  cil_seq_node cur, next, prev;
  cil_stmt stmt = cil_iter_stmt (*i);
...
cil_stmt_iterator
cil_iter_for_stmt (cil_stmt stmt)
{
  cil_stmt_iterator i;

  for (i = cil_iter_start_bb (cil_bb (stmt)); !cil_iter_end_p (i); cil_iter_next (&i))
    if (cil_iter_stmt (i) == stmt)
      return i;
...
void
cil_iter_move_after (cil_stmt_iterator *from, cil_stmt_iterator *to)
{
  cil_stmt stmt = cil_iter_stmt (*from);
  cil_iter_remove (from);

  /* We must have CIL_ITER_NEW_STMT here, as cil_iter_move_after is sometimes used to
     move statements to an empty block.  */
  cil_iter_insert_after (to, stmt, CIL_ITER_NEW_STMT);
}
...
void
cil_iter_move_before (cil_stmt_iterator *from, cil_stmt_iterator *to)
{
  cil_stmt stmt = cil_iter_stmt (*from);
  cil_iter_remove (from);

  /* For consistency with cil_iter_move_after, it might be better to have
     CIL_ITER_NEW_STMT here; however, that breaks several places that expect
     that TO does not change.  */
  cil_iter_insert_before (to, stmt, CIL_ITER_SAME_STMT);
}

/* Move the statement at FROM to the end of basic block BB.  */

void
cil_iter_move_to_bb_end (cil_stmt_iterator *from, basic_block bb)
{
  cil_stmt_iterator last = cil_iter_last (cil_bb_seq (bb));

  /* Have to check cil_iter_end_p because it could be an empty block.  */
  if (!cil_iter_end_p (last))
    cil_iter_move_before (from, &last);
  else
    cil_iter_move_after (from, &last);
}
...
/* Insert the sequence delimited by nodes FIRST and LAST before
   ...
   (see enum cil_iter_iterator_update).
...
static void
cil_iter_insert_seq_nodes_before (cil_stmt_iterator *i, cil_seq_node first,
			     cil_seq_node last, enum cil_iter_iterator_update m)
{
  basic_block bb;
  cil_seq_node cur = i->ptr;

  if ((bb = cil_iter_bb (*i)) != NULL)
  ...
  switch (m)
    {
    case CIL_ITER_NEW_STMT:
    case CIL_ITER_CONTINUE_LINKING:
      i->ptr = first;
      break;
    case CIL_ITER_SAME_STMT:
      break;
    default:
      gcc_unreachable ();
    }
...
static void
cil_iter_insert_seq_nodes_after (cil_stmt_iterator *i, cil_seq_node first,
			    cil_seq_node last, enum cil_iter_iterator_update m)
{
  basic_block bb;
  cil_seq_node cur = i->ptr;
