...
simplify_cond_branch (cil_stmt_iterator *csi, cil_stack stack)
{
  ...
  if (cil_iter_bb (*csi)->next_bb == then_bb)
    {
      type = cil_stack_top (stack);

      if (cil_integer_p (type) || cil_pointer_p (type))
	{
	  switch (cil_opcode (cil_iter_stmt (*csi)))
	    {
...
simp_cond (void)
{
  ...
  FOR_EACH_BB (bb)
      ...
      while (!cil_iter_end_p (csi))
	{
	  ...
	  if (cil_cond_branch_p (stmt) && !cil_iter_one_before_end_p (csi))
	    {
	      cil_iter_next (&csi);

	      if (cil_opcode (cil_iter_stmt (csi)) == CIL_BR)
		{
		  cil_iter_prev (&csi);
