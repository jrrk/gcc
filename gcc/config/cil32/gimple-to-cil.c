...
/******************************************************************************
 * GIMPLE/legacy functions subsequently removed from compiler
 ******************************************************************************/
...
static tree build_resx (int region_number)
{
  tree t;
...
static tree_ann_common_t create_tree_common_ann (tree t)
{
  ...
  ann = XCNEW (struct tree_ann_common_d);
...
static tree gimple_to_tree (gimple stmt)
{
  tree t;
  int rn;
  tree_ann_common_t ann;
...
static void release_stmt_tree (gimple stmt, tree stmt_tree)
{
  tree_ann_common_t ann;
...
gen_integer_cst (cil_stmt_iterator *csi, tree cst)
{
  ...
  cil_iter_insert_after (csi, stmt, CIL_ITER_CONTINUE_LINKING);
}
...
gen_addr_expr (cil_stmt_iterator *csi, tree node)
{
  ...
  switch (TREE_CODE (node))
    ...
    case VIEW_CONVERT_EXPR:
      ...
      cil_iter_insert_after (csi, cil_build_stmt (CIL_CONV_I), CIL_ITER_CONTINUE_LINKING);
      break;
...
cil_seq_num_instr (cil_seq seq)
{
  ...
  for (csi = cil_iter_start (seq); !cil_iter_end_p (csi); cil_iter_next (&csi))
    i++;
...
expand_init_to_cil_seq (tree decl, tree init)
{
  ...
  if (cil_choose_init_method (num_seq1, num_seq2, size, same))
      ...
      else /* TARGET_LITTLE_ENDIAN */
	{
      ...
      cil_set_prefix_unaligned (stmt, 1);
      cil_iter_insert_after (&csi, stmt, CIL_ITER_CONTINUE_LINKING);
      cil_iter_insert_seq_after (&csi, seq2, CIL_ITER_CONTINUE_LINKING);
    }
  else
    cil_iter_insert_seq_after (&csi, seq1, CIL_ITER_CONTINUE_LINKING);
...
gen_goto_expr (cil_stmt_iterator *csi, tree node)
{
  ...
  if (TREE_CODE (label_decl) != LABEL_DECL)
  ...
  else
    {
      ...
      if (cil_iter_bb (*csi)->next_bb != dest_bb)
	{
...
gen_cond_expr (cil_stmt_iterator *csi, tree node)
{
  ...
  extract_true_false_edges_from_block (cil_iter_bb (*csi), &true_edge, &false_edge);
...
gen_bit_and_expr (cil_stmt_iterator *csi, tree node)
{
  ...
  if (TREE_CODE (op0) == INTEGER_CST
	...
	default:
      ...
      cil_iter_insert_after (csi, cil_build_stmt (opcode), CIL_ITER_CONTINUE_LINKING);
...
gimple_to_cil_node (cil_stmt_iterator *csi, tree node)
{
  ...
  switch (TREE_CODE (node))
	...
	{
	  cil_iter_insert_seq_after (csi, expand_init_to_cil_seq (op0, op1),
				CIL_ITER_CONTINUE_LINKING);
	}
      else
	...
	{
	  cil_iter_insert_after (csi, cil_build_stmt (CIL_CONV_I),
			    CIL_ITER_CONTINUE_LINKING);
	}
...
process_initializers (void)
{
  ...
  cil_stmt_iterator csi = cil_iter_start (seq);
  cil_stmt_iterator bb_csi = cil_iter_start_bb (single_succ (ENTRY_BLOCK_PTR));
  tree cell;
  ...
  for (cell = cfun->local_decls; cell; cell = TREE_CHAIN (cell))
	...
	{
	  cil_iter_insert_seq_after (&csi, expand_init_to_cil_seq (var, init),
				CIL_ITER_CONTINUE_LINKING);
	}
    }

  cil_iter_insert_seq_before (&bb_csi, seq, CIL_ITER_SAME_STMT);
}
      ...
      for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
	{
	  ...
	  switch (TREE_CODE (node))
		...
		if (!VOID_TYPE_P (TREE_TYPE (fun_type)))
		  {
		    cil_iter_insert_after (&csi, cil_build_stmt (CIL_POP),
				      CIL_ITER_CONTINUE_LINKING);
		  }
	      }
	      break;
	  ...
	  for (; !cil_iter_end_p (prev_csi); cil_iter_next (&prev_csi))
	    cil_set_locus (cil_iter_stmt (prev_csi), gimple_location (gimple_node));
	}
